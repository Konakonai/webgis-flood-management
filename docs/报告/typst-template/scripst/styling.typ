...
#let stypar(lang: "zh", par-indent: 2em, leading: 1em, spacing: 1.1em, body) = {
  ...
  // Keep narrative paragraphs aligned, including the first paragraph after a heading.
  set par(
...
#let styfigure(counter-depth: 2, font: font.caption, body) = {
  set figure(gap: 0.5cm, numbering: none)
  show figure: it => [
    #set par(first-line-indent: 0em)
    #v(2pt)
    #set text(font: font)
...
#let styraw(font: font.raw, body) = {
  ...
  show raw.where(block: true): set par(first-line-indent: 0em, leading: 0.4em)
